...
export function parseFare(fareString) {
  ...
  const fare_amount = parseFloat(fareString);

  if (isNaN(fare_amount)) return -1;
...
export function calculateSurge(baseFare, surgeMultiplier) {
  if (typeof baseFare !== "number" || typeof surgeMultiplier !== "number")
    return 0;
  if (baseFare < 1 || surgeMultiplier < 1) return 0;
...
export function findCheapestAndCostliest(...fares) {
  ...
  fares = fares.filter((fare) => typeof fare === "number");
  if (fares.length === 0) return null;
  let cheapest = Math.min(...fares);
  let costliest = Math.max(...fares);

  if (isNaN(cheapest) || isNaN(costliest) || cheapest === 0 || cheapest === 0)
    return null;
  return { cheapest, costliest };
}
